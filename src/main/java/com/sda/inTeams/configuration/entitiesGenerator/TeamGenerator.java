...
public class TeamGenerator {
    ...
    public static List<Team> generateTeams(int size) {
        List<Team> users = new ArrayList<>();
        IntStream.range(0, size).forEach(ind -> users.add(generateSingleTeam()));
        return users;
    }

    public static Team generateSingleTeam() {
        return Team.builder()
                .name("Team " + StringUtilities.getRandomWord() + " " + StringUtilities.getRandomNumberAsString(3))
                .build();
    }
}
